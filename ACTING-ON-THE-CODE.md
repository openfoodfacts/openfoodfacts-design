# Acting on the Code

This document provides quick links to the key code locations for implementing design changes across Open Food Facts products.

## Server (Website)
- CSS: https://github.com/openfoodfacts/openfoodfacts-server/tree/main/scss
- Templates: https://github.com/openfoodfacts/openfoodfacts-server/tree/main/templates
- Icons: https://github.com/openfoodfacts/openfoodfacts-server/tree/main/icons — many sourced from [Google Material Symbols and Icons](https://fonts.google.com/icons), plus custom icons by [@stephanegigandet](https://github.com/stephanegigandet)

## Mobile App (smooth-app)
- Source: https://github.com/openfoodfacts/smooth-app
- Food Icons (foodicons) — a handcrafted icon font by [@g123k](https://github.com/g123k): https://github.com/openfoodfacts/smooth-app/tree/develop/packages/smooth_app/assets/fonts
- Other assets (images, etc.): https://github.com/openfoodfacts/smooth-app/tree/develop/packages/smooth_app/assets

## Explorer (Next-gen Frontend)
- Source: https://github.com/openfoodfacts/openfoodfacts-explorer

## Hunger Games
https://github.com/openfoodfacts/hunger-games


## Wiki
- Hasn't been styled specifically
- Config at https://github.com/openfoodfacts/openfoodfacts-infrastructure/blob/develop/confs/ovh1-reverse-proxy/nginx/conf.d/wiki.conf

## Keycloak (login system)
- Incoherent styling
- https://github.com/openfoodfacts/openfoodfacts-auth/tree/main/theme

## Nutri-Patrol (moderation system)
- https://github.com/openfoodfacts/nutripatrol-frontend

## Open Prices
- https://github.com/openfoodfacts/open-prices-frontend
- Not very convergent with the graphic charter so far

## Search engine
- https://github.com/openfoodfacts/search-a-licious

## Contents (for illustration)
- https://github.com/openfoodfacts/openfoodfacts-web

## Webcomponents (visual aspects are mostly abstracted away)
- https://github.com/openfoodfacts/openfoodfacts-webcomponents
